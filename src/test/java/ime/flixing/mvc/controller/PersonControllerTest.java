...
class PersonControllerTest {
	...
	void PersonController_getAllPerson_ReturnOptListPerson() {
		...
		doReturn(personList).when(personDaoImpl).getAllPerson();
		...
		verify(personDaoImpl,times(1)).getAllPerson();		
	...
	void PersonController_getPersonById_ReturnPerson() {
		...
		try( MockedStatic<Checker>mockChecker = Mockito.mockStatic(Checker.class) ){
			...
			doReturn(personTest).when(personDaoImpl).getPersonById(Mockito.anyLong());
			...
			verify(personDaoImpl,times(1)).getPersonById(Mockito.anyLong());
	...
	void PersonController_savePerson_ReturnPerson() {
		...
		try( MockedStatic<Checker>mockChecker = Mockito.mockStatic(Checker.class) ){
			...
			doReturn(personTest).when(personDaoImpl).savePerson(Mockito.any(Person.class));
			...
			verify(personDaoImpl,times(1)).savePerson(Mockito.any(Person.class));
	...
	void PersonController_updatePerson_ReturnPerson() {
		...
		try( MockedStatic<Checker>mockChecker = Mockito.mockStatic(Checker.class) ){
			...
			mockChecker.when( () -> Checker.checkSurname(Mockito.anyString()) ).thenReturn(true);
			doReturn(personTest).when(personDaoImpl).getPersonById(Mockito.anyLong());
			doReturn(personTest).when(personDaoImpl).updatePerson(Mockito.anyLong(), Mockito.any(Person.class));
			...
			verify(personDaoImpl,times(1)).getPersonById(Mockito.anyLong());
			verify(personDaoImpl,times(1)).updatePerson(Mockito.anyLong(), Mockito.any(Person.class));
			mockChecker.verify(() -> Checker.checkDigits(Mockito.anyString()), times(1));
			mockChecker.verify(() -> Checker.checkName(Mockito.anyString()), times(1));			
			mockChecker.verify(() -> Checker.checkSurname(Mockito.anyString()), times(1));		
		}	
	...
	void PersonController_deletePerson_ReturnInt() {
		...
		try( MockedStatic<Checker>mockChecker = Mockito.mockStatic(Checker.class) ){
			...
			mockChecker.when( () -> Checker.checkDigits(Mockito.anyString()) ).thenReturn(true);
			doReturn(personTest).when(personDaoImpl).getPersonByIdEagger(Mockito.anyLong());
			doNothing().when(personDaoImpl).deletePerson(Mockito.anyLong());			
			...
			verify(personDaoImpl,times(1)).getPersonByIdEagger(Mockito.anyLong());
			verify(personDaoImpl,times(1)).deletePerson(Mockito.anyLong());
			mockChecker.verify(() -> Checker.checkDigits(Mockito.anyString()), times(1));
		}	
	}
